...
## 基本概念
### Prompt
Prompt最初是NLP（自然语言处理）研究者为下游任务设计出来的一种任务专属的输入模版，类似于一种任务对应一种Prompt。我每一次访问大模型的输入为一个Prompt，大模型返回给我们的结果则被称为Completion
...
## 向量化模型
### 什么是向量化
向量化是将非结构化数据（如文本，图像，音频等）转换为数值向量表示的过程
向量是数学空间中的点，能够捕捉原始数据的语义特征和关系，用于语义搜索、推荐、聚类、分类、异常检测等下游任务
+ 文本化向量：通过词嵌入模型将句子转换为数值向量
+ 图像化向量：用CNN提取特征，将图片转换为数值向量
### 向量数据库
向量数据库是专门处理高维向量数据的系统，能将文本、图形等非结构化数据转化为数值向量存储，并通过相似性搜索实现精准匹配。
其核心能力包括：
1. 语义级检索：通过计算向量间距离（如余弦相似度）找到最接近的语义表达
2. 多模态支持：直接处理图片、音频等复杂数据
3. 高效索引：采用HNSW、IVF-PQ等算法实现快速查询
#### Redis向量数据库
Redis 8.0通过Vector Set数据结构提供向量存储能力
安装Redis-stack替代redis
```
...
public class AudioTranscriptController {  
    ...
    public ResponseEntity<String> transcription(@RequestParam("input") String input) {  
        try{  
            if(input == null || input.trim().isEmpty()){  
                return ResponseEntity.badRequest().body("输入参数不能为空");  
            }  
            Resource resource = new UrlResource(input);  
