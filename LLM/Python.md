...
## 向量存储
### 向量化
我们知道，计算机不能理解人类的语言。本质上，它只能进行各种各样的数据计算。所以要想让计算机理解人类的语言，我们就得将语言拆成Token，再将Token数据化，转成一串串的数字，这样才能让计算机通过计算的方式去理解Token之间的相似性，从而进一步理解语言背后的语义信息
我们可以将一个文本转换成多维向量，然后通过向量之间的计算，来分析文本与文本之间的相似性
向量化就是将非结构化或结构化的数据，通过数学模型转换成地位、稠密的数值向量的过程。这个过程的核心目标是让计算机能够用数学方式理解、计算和比较数据的含义
对于两个词之间，我们通过计算向量之间的余弦相似度，就能判断出两个词的词义是否相近，余弦相似度的取值为\[0,1]，值越大，说明两个向量之间的夹角越小，两个词也就越相近
$$
...
embeddings = em_llm.embed_documents(text)  
...
```text
文本：苹果
向量：[0.015341216698288918, -0.025428418070077896, -0.008811954408884048, 0.05595782399177551, 0.0006985960644669831]
文本：香蕉
向量：[0.0038208216428756714, 0.027792422100901604, -0.012286439538002014, 0.016924872994422913, -0.00036227816599421203]
文本：汽车
向量：[0.008696823380887508, -0.019546153023838997, -0.0035542405676096678, 0.03978666290640831, 0.0010285165626555681]
```
**向量化的主要作用**
- 统一数据表示：文本、图像、音频、用户行为等不同模态的数据，最终都变成了同一种格式——数值向量，为跨模态分析提供了基础
- 支撑语义检索：传统搜索时关键词匹配，而向量搜索是语义匹配
- 高效计算与存储：稠密向量相比稀疏的编码，维度更低、信息密度更高，极大降低了计算复杂度和存储成本，是大模型时代的基础数据格式
- 驱动AI应用：大模型的知识库检索、推荐系统、人脸识别、语音识别、聚类分析等核心功能，底层都依赖向量化技术
### 向量数据库
**向量存储**
