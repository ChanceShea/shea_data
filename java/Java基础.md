...
- ArrayList是应用更加广泛的动态数组实现，它本身不是线程安全的，性能更好。与Vector类似，ArrayList也可以根据需要调整容量，不过两者的调整逻辑有区别，Vector在扩容时会提高1倍，而ArrayList是增加50%。**JDK1.7之前，初始化ArrayList时如果不传参数，会直接创建一个大小为10的ArrayList，JDK1.7之后，采用懒加载的方式，首次创建时创建的是一个大小为0的List，只有在第一次调用add方法时，才会扩容为大小为10的List。当数组存满时，会将ArrayList扩容到原来的1.5倍，即增加50%**
