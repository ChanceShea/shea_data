...
# JMM
JMM即 Java Memory Model，是专门解决多线程并发问题的一套规则。定义了主存，本地内存抽象概念。主存里面主要存储一些共享变量，本地内存里主要存储一些共享变量的副本，交由线程修改，本地内存里的变量是每个线程私有的
体现在以下几方面
...
基于AQS实现的锁和同步工具，主要可以分为 **锁（Lock）** 和 **同步器（Synchronizer）** 两大类
## 锁（Lock）
这类工具主要实现`Lock`接口，用于提供比`synchronized`更灵活的锁机制
- **ReentrantLock**，独占锁，支持锁重入，即一个线程可以多次获取同一把锁而不会死锁。同时支持公平和非公平两种获取策略
- **ReentrantReadWriteLock**，读写锁，包含读锁（共享锁）和写锁（独占锁）。读锁可以被多个线程同时持有，可以多线程同时读数据；写锁只能被单线程持有，适用于读多写少的场景
- **StampedLock**，乐观锁，在读写锁的基础上增加了乐观读模式，性能更高
## 同步器（Synchronizer）
这类工具不直接实现`Lock`接口，但是提供了独特的线程协调功能
- **Semaphore**，共享锁，即信号量，通过一个计数器控制同时访问特定资源的线程数。维护了一个许可计数器，线程在访问资源时要通过`tryAcquire()`方法获取许可，如果有可用许可，则获取成功，并将计数器减1，否则线程需要等待，直到有其他线程通过`realease()`方法释放许可
- **CountDownLatch**：共享锁，它允许一个或多个线程等待其他线程完成一组操作后再继续执行。通过一个计数器来实现，计数器初始化为线程的数量，每个线程完成任务后可以调用`countDown()`方法将计数器减1，当计数器为0时，等待的线程可以继续执行
- **CylicBarrier**：共享锁，让一组线程互相等待，直到所有线程都到达某个屏障点后，再一起继续执行。与`CountDownLatch`不同的是，`CylicBarrier`可以重复使用，当所有线程都通过屏障后，计数器会重置，可以再次用于下一轮的等待
# 杂项
## blocked和waiting的区别
...
## 乐观锁
### 实现乐观锁
1. CAS操作：CAS是乐观锁的基础，Java提供了各种原子变量类，这些类使用CAS操作实现了线程安全的原子操作，可以用来实现乐观锁
2. 版本号控制：增加一个版本号字段记录数据更新时候的版本，每次更新时递增版本号。在更新数据时，同时比较版本号是否相同，若当前版本和与更新前获取的版本号不一致，则更新失败，否则更新成功
3. 时间戳：使用时间戳记录数据的更新时间，在更新数据时，比较时间戳，如果当前时间戳大于数据的时间戳，则说明数据已经被其他线程更新，更新失败
### CAS缺点
1. ABA问题：CAS更新过程中，读取到的是A，准备赋值是依然是A，但是实际上，在这个过程中，如果A的值被改成了B，然后又被改回了A，这种情况下就无法察觉。但是ABA问题大部分场景下都不影响并发的最终结果。Java中有`AtomicStampedReference`来解决这个问题，它加入了预期标志和更新后标志两个字段，更新时不光检查值，还要检查当前的标志是否等于预期标志，全部相等才会更新
2. 循环时间长，开销大：自旋CAS的方式如果长时间不成功，会给CPU带来很大的开销
3. 只能保证一个共享变量的原子操作：只对一个变量操作可以保证原子性，但多个变量不行，多个变量可以通过AtomicReference来处理或使用锁synchronized来实现
## volatile
volatile有两个作用
- 保证变量对所有线程的可见性：当一个变量被声明为volatile时，它会保证这个变量的写操作会被立即刷新到主存中，而对这个变量的读操作会直接从主存中读取，从而保证了多线程环境下对该变量访问的可见性，不会受到各线程工作内存的影响
- 禁止指令重排序优化：volatile关键字在java中主要通过内存屏障来禁止特定类型的指令重排序
	**写写屏障**：在对volatile变量执行写操作之前，会插入一个写屏障。这确保了在该变量写操作之前的所有普通写操作都已完成，防止了这些写操作被移到volatile写操作之后
	**读写屏障**：在对volatile变量执行读操作之后，会插入一个读屏障。它确保了volatile变量的读操作之后的所有普通读操作都不会被提前到volatile读之前执行，保证了读取到的数据是最新的
	**写读屏障**：这是最重要的一个屏障，它发生在volatile写之后和volatile读之前。这个屏障确保了volatile写操作之前的所有内存操作都不会被重排序到volatile读之后，同时也确保了volatile读操作之后的所有内存操作都不会被重排序到volatile写之前
### 指令重排序
在执行程序时，为了提高性能，处理器和编译器通常会对执行进行重排序，但是重排序要满足下面两个条件才能进行
- 单线程环境下不能改变程序运行的结果
- 存在数据依赖关系的不允许重排序
所以重排序并不会对单线程有影响，只会破坏多线程的语义
### volatile和synchronized
synchronized解决了多线程访问共享资源时可能出现的竞态条件和数据不一致的问题，保证了线程安全性。synchronized是一种排他性的同步机制，保证了多个线程访问共享资源时的互斥性，即同一时刻只允许一个线程访问共享资源。通过对代码块或方法添加synchronized关键字来实现同步
volatile解决了变量在多线程环境下的可见性和有序性问题，确保了变量的修改对其他线程是可见的。volatile是一种轻量级的同步机制，用来保证变量的可见性和禁止指令重排序。当一个变量被声明为volatile时，线程在读取该变量时会直接从内存中读取，而不会使用缓存，同时对该变量的写操作会立刻刷回主内存，而不是缓存在本地内存中
## 公平锁和非公平锁
- 公平锁：指多个线程按照申请锁的顺序来获取锁，线程直接进入队列中排队，队列中的第一个线程才能获得锁。公平锁的优点在于各个线程公平平等，每个线程等待一段时间后，都有执行的机会，但是整体的执行速度更慢，吞吐量更小
- 非公平锁：多个线程加锁时直接尝试获取锁，能抢到锁的线程直接占有锁，抢不到锁的才会在等待队列的队尾等待。非公平锁的优势在于整体执行速度更快，吞吐量更大，但同时也可能产生线程饥饿的问题，也就是说如果一直有线程插队，那么在等待队列中的线程可能长时间得不到运行
### 为什么非公平锁的吞吐量大
