...
## RocketMQ
RocketMQ中有以下几个主要的角色
- Producer：消息发送者。Producer跟Name Server的任意一个节点建立长连接，定期从Name Server拉取Topic路由信息
- Broker：暂存和传输消息，RocketMQ中实现消息暂存和发送消息的主要单元。即处理生产者的消息写入请求，支持实时消费与查询。Broker采用主从集群，实现多副本存储和高可用。每个Broker节点都要跟所有的Name Server节点建立长连接，定义注册Topic路由信息和发送心跳。跟所有Name Server建立连接，就不会因为单个Name Server挂了影响Broker使用。Broker主从模式中，Slave节点主动从Master节点拉取消息
	Broker中，Master Broker用于处理生产者的消息写入请求，支持实时消费与查询；Slave Broker从Master异步/同步复制数据，提供灾备冗余与故障切换能力
- NameServer充当了RocketMQ集群的注册中心，它负责为负所有Broker的网络地址，主题的路由信息以及消费者的订阅关系
	当生产者发送消息到RocketMQ集群时，它首先需要与NameServer进行交互，**以获取特定主题的消息队列在哪些Broker上，并且按照一定的负载均衡策略选择一个合适的Broker发送消息**
	消费者订阅主题之前，也需要与NameServer联系，**以获取订阅主题的路由信息，包括在哪个Broker上有该主题的消息队列以及消费者从哪些队列消费**
	...
	**NameServer是RocketMQ集群中的关键组件，用于管理和维护Broker集群消息、主题的路由信息和消费者的订阅关系。它提供了生产者和消费者与RocketMQ集群交互的入口点，为消息的发送和消费提供了必要的服务**
- Topic：区分消息的种类，用于标识同一类业务逻辑的消息，主题通过TopicName来做唯一标识和区分。**一个发送者可以发送消息给一个或多个Topic，一个消息的接收者可以接收一个或者多个Topic**。主题内部由多个队列组成，消息的存储和水平扩展能力最终是由队列实现的
- Message Queue：相当于topic的分区，可以理解为一个逻辑上的消息分组，它将消息按照某种规则进行划分和管理。用于并发发送和接收消息。Producer发送的消息会在Broker的MessageQueue中保存，当然MessageQueue不保存物理消息，而是保存消息的偏移量。有了MessageQueue，Topic的消息就可以在Broker中实现分布式存储
	每个MessageQueue都有一个唯一的标识符，通常由主题和队列ID组成。在RocketMQ中，一个主题可以被划分为多个消息队列，**每个队列只会被一个消费者进行消费**。这种划分可以提高消息的并行处理能力，允许多个消费者同时处理不同的消息队列，从而提高整体的消息处理吞吐量。使用消息队列，可以实现负载均衡和并行处理，同时还能保证消息的有序性
	例如，当一个主题有大量的消息需要处理时，RocketMQ将根据一定的策略将这些消息均匀地分发到多个消息队列中，以使多个消费者可以并行地消费消息。通过使用消息队列，RocketMQ能够有效地处理大量的消息，并提供高吞吐量和可靠性。消息队列的使用对于构建可扩展性和高性能的分布式消息系统是非常重要的
- Consumer：消息接收者。Consumer跟NameServer的任意一个节点建立长连接，定期从NameServer拉取Topic路由消息
### 消费消息
在消费者客户端消费时，有两种订阅消息的方式，分别是并发消费和顺序消费
- 并发消费：因为topic分了多个区，允许同一消费者组内多个线程并行拉取不同队列的消息。按队列轮询分配，多个线程可同时处理不同队列的消息，同一主题的消息可能被多个线程无序处理
- 顺序消费：消费一个主题时，消费消息的顺序和消息发送的顺序一致。如果一个Topic有多个队列，是不可能达成Topic级别的顺序消费的，因为无法控制哪个队列的消息最先被消费。Topic只有一个队列的情况下能够实现Topic级别的顺序消费
顺序消息分为了全局顺序性和分区顺序性
- 全局顺序性：需要限制Topic仅含一个队列，单线程生产与消费，适用于低吞吐量场景
- 分区顺序性：通过将同一业务标识的消息路由至同一消息队列，确保队列内消息顺序与发送顺序一致。顺序消费的大致原理是依靠两组锁，一组在Broker端，锁定队列和消费者的关系，保证同一时间只有一个消费者在消费；在消费者端也有一组锁以保证消费的顺序性
RocketMQ顺序消息的实现，本质上是基于队列级的有序性，而非依赖生产者的同步发送模式。在Spring Cloud Stream框架中，这一机制通过`PartitionKeyExtractorStrategy`接口落地。开发者需实现该接口并重写`extractKey`方法，从消息体中提取业务分区键，随后将自定义实现注册为SpringBean。框架以此确保具有相同键值的消息被路由至同一MessageQueue，既保障了消费端的顺序处理，又通过分区策略实现了系统负载的有效均衡
### 消息去重
RocketMQ的消息去重，本质上是在业务层面实现消息的幂等消费。因为RocketMQ的设计哲学是“至少消费一次”，它保证消息不丢失，但并不保证消息不重复
**为什么会有消息重复**
- 生产者发送阶段：生产者发送消息后，因网络闪断未收到服务端的成功响应，会触发重试机制再次发送，导致Broker中存在多条内容相同的消息
- 消费者消费阶段：消费者处理完业务逻辑后，因网络问题未能及时向Broker提交消费进度，Broker会认为消费失败并重新投递该消息
- 其他场景：消费者组发生重平衡、Broker异常重启导致Offset未持久化、主从同步失败等，都会造成重复消费
**消息重复的危害**
- 资金损失：用户下单支付，因网络问题导致支付消息被重复消费，系统可能会执行多次扣费，导致用户账户被重复扣费
- 数据一致性错误：在订单创建的场景，如果业务代码未做校验，重复消费会导致数据库插入多条完全相同的订单记录，造成数据冗余和统计失真
- 资源浪费：每条消息的消费都需要经历网络传输、反序列化、业务逻辑执行等过程，重复消息会占用这些计算资源。对于一些第三方接口（短信、邮件发送等），重复调用可能还会触发服务商的限流策略，导致正常请求也被拒绝
**解决方案**
解决重复消费问题的核心思路是：无论收到多少次重复消息，业务处理的结果都应该是唯一的。业界通用的做法是使用业务的唯一标识作为去重依据
虽然每条消息都有唯一的Message ID，但在生产者发送失败重试的场景下，同一条业务消息可能会产生不同的Message ID。因此，Message ID不能作为可靠的去重依据
解决方案的核心，就是在消费端实现幂等处理（即同一条消息被消费多次，产生的结果和消费一次完全相同），具体方案可以结合业务唯一ID和数据库唯一索引或Redis锁来实现
### 事务消息
事务消息是RocketMQ提供的一个重要特性，可以利用事务消息轻松实现分布式事务。RocketMQ在消息定义的基础上，对事务消息扩展了两个相关的概念
- Half(Prepare) Message：半消息是一种特殊的消息类型，该状态的消息暂时不能被Consumer消费。当一条事务消息被成功投递到Broker上，但是Broker并没有接收到Producer发出的二次确认时，该事务消息就处于暂时不可消费的状态，该状态的事务消息被称为半消息
- Message Status Check：由于网络抖动，Producer重启等原因，可能导致Producer向Broker发送的二次确认消息没有成功送达。如果Broker检测到某条事务消息长时间处于半消息状态，则会主动向Producer端发起回查操作，查询该事务消息在Producer端的事务状态。可以看出，Message Status Check主要用于解决分布式事务中的超时问题
- 事务ID：RocketMQ中的事务ID和消息的key都是用来识别消息的唯一标识。在RocketMQ中，当你使用事务消息特性时，会为每个事务创建一个事务ID。这个ID用于确保事务消息的唯一性，即使在消息被提交之后，也能通过这个ID找到对应的消息
事务消息采用2PC模型
- 1PC（发送预备消息）：生产者发送半事务消息至消息队列，此时消息对消费者不可见
- 2PC（事务状态确认）：生产者执行本地事务后，向消息队列提交Commit或Rollback指令，决定消息是否投递
1. 当应用模块遇到需要发送事务消息的场景，会先发送prepare消息给MQ
2. prepare消息发送成功后，应用模块执行数据库事务（本地事务）
3. 根据数据库事务执行的结果，再返回commit或rollback给MQ
4. 如果是commit，MQ把消息下发给Consumer端，如果是rollback，直接删除prepare消息
5. 第三步的执行结果如果没有响应，或者是超时的，启动定时任务回查事务状态，处理结果同第四步
6. MQ消费的成功机制由MQ自己保证
### 事务回查
事务消息回查机制在分布式系统中起到关键的作用，主要体现在以下方面
- 解决事务状态不确定性：当生产者发送预备消息后，可能因服务崩溃、网络故障或事务执行超时导致无法主动提交最终状态。此时回查机制通过消息队列定时扫描未确认的半事务消息，主动向生产者发起状态回查，确保每条消息都能获得确定性处理结果。
- 保障系统可用性：若未实现回查机制，异常场景下大量半事务消息会长期滞留队列，占用存储资源并阻塞后续消息处理；通过失败重试策略，即使首次状态查询失败也能通过后续重试恢复处理流程，降低单点故障影响
- 实现最终一致性：回查机制作为两阶段提交的补充，在生产者未响应时通过预设规则或业务逻辑重试，确保跨系统操作的最终一致性。结合事务日志，回查可通过唯一事务ID关联业务状态，修复因异步处理导致的数据不一致问题
