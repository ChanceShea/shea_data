...
## 58. GC垃圾回收
### 垃圾回收算法
**标记清除**：分为标记和清除两个阶段。先标记所有存活对象，再统一回收未被标记的对象。实现简单，但是很容易产生内存碎片
**复制**：将内存分为两块，每次只使用一块内存，当一块用完，将存活对象复制到另一块，然后一次性清空原区域。实现简单，运行高效，不会产生内存碎片。但是内存利用率低，始终有一半内存闲置
**标记整理**：标记阶段和标记清除相同，后续是将所有存活对象移到内存区域的一端，然后清理边界以外的内存区域。不会产生内存碎片。但是移动对象需要暂停用户线程，效率相对较低
**分代回收**：根据对象的存货周期的不同，将堆划分为新生代和老年代，对不同区域采用不同的回收算法。提高了整体的回收效率，但是需要维护分代信息，设计相对复杂
### 垃圾回收器
**Serial**：新生代垃圾回收器，使用复制算法。单线程回收，进行垃圾回收时会触发STW。Client模式下默认新生代收集器
**ParNew**：新生代垃圾回收器，使用复制算法。Serial的多线程并行版本，常与CMS老年代收集器搭配使用
**Parallel Scavenge**：新生代垃圾回收器，使用复制算法。多线程并行回收，以吞吐量优先为目标，是Java8及之前版本的Server模式默认的新生代回收器
**Serial Old**：老年代垃圾回收器。使用标记整理算法。Serial的老年代版本，单线程
**Parallel Old**：老年代垃圾回收器，采用标记整理算法。Parallel Scavenge的老年代版本，多线程，吞吐量优先
**CMS**：老年代垃圾回收器。采用标记清除算法。首个真正意义上的并发收集器，追求低停顿。主要缺点是会产生内存碎片。JDK9之后被废弃
**G1**：整个堆都能回收。整体采用标记整理算法；新生代采用复制算法。将整个堆划分为多个Region，通过停顿预测模型实现可预测的停顿时间。从JDK9开始成为默认垃圾回收器
**ZGC**：整个堆都能回收，采用标记复制算法。追求超低延迟，与堆的大小无关，通过染色指针和读屏障等技术实现。从JDK15开始可以使用
**Shenandoah**：整个堆都能回收，采用并发标记-整理（复制）算法。与G1类似，丹江对象复制阶段也变为并发。进一步降低停顿。从JDK12开始引入
## 59. Spring循环依赖
...
## 60. HTTPS
HTTPS的安全核心是**混合加密**与**数字证书**的结合。前者解决了数据加密传输的问题，后者则确保了用于加密的公钥是真实可信的
**混合加密**
HTTPS同时采用了非对称加密和对称加密两种方式
- **非对称加密**：它使用配对的公钥和私钥。公钥可以公开，用于加密；私钥必须保密，用于解密。虽然安全，但是计算复杂、速度慢，不适合加密大量数据。在HTTPS中，它主要用于在握手阶段安全地交换一个临时的会话密钥
- **对称加密**：使用同一个密钥进行加密和解密，运算速度快，适合加密大量数据。在HTTPS中，一旦双方通过非对称加密安全地协商出会话密钥，后续所有的数据传输都将使用这个密钥进行高效的对称加密
这种**非对称加密交换密钥，对称加密传输数据**的组合方式，兼顾了安全与效率
**数字证书**
为了防止公钥被伪造，关键就在于数字证书和证书颁发机构(CA)体系。工作流程如下
- **服务器申请证书**：网站所有者向受信任的CA机构申请证书。CA在严格审核其身份后，会为服务器生成一个数字证书。这个证书里绑定了网站域名、服务器公钥等重要信息
- **CA签名防伪**：CA会使用自己的私钥对该证书的内容进行数字签名。这个签名如同一个防伪标签，能确保证书的内容未被篡改
- **浏览器内置信任**：操作系统和浏览器会内置一个“受信任的根证书列表”，里面包含了全球知名CA机构的公钥
- **客户端验证证书**：当浏览器连接到服务器时，服务器会出示其数字证书。浏览器会进行以下校验。首先检查证书的域名、有效期等信息是否匹配；然后使用内置的CA公钥，去解密证书上的数字签名。如果解密成功，且与证书内容匹配，就证明该证书由可信CA颁发且未被篡改的；并且会确认该证书是否还在有效期内，未被CA提前吊销
**一次HTTPS握手需要以下几步**
- 浏览器和服务器互相问候，并交换各自的随机数。同时，服务器会把自己的数字证书发给浏览器
- 浏览器会严格检查服务器的数字证书。验证通过后，才会信任证书里的公钥
- 浏览器生成一个随机数，并用刚从证书里得到的服务器公钥对它进行加密，然后发给服务器。由于只有服务器有对应的私钥，因此只有服务器能解开
- 此时，浏览器和服务器都拥有了三个相同的随机数。双方用相同的算法，各自独立计算出一个相同的会话密钥
- 双方确认无误后，就都用这个会话密钥来加密后续的所有数据，HTTPS的安全通道正式建立
## 61. 高并发场景下，Tomcat线程池中的线程被消耗完了该怎么办
在请求到达线程池之前，Tomcat的连接器还提供了两层缓冲，作为第一道防线
- **acceptCount**：等待队列长度。当所有线程都在忙时，新连接会被放入操作系统的TCP等待队列中。这个队列的长度由`acceptCount`控制，默认为100
- **maxConnections**：最大连接数。这是Tomcat能同时接受的最大连接总数。当达到此数值后，Tomcat会拒绝接收新的连接，直到现有连接数下降
Tomcat的线程池与标准的JDK线程池在处理流程上有不同的，它针对Web应用的IO密集型特点进行了优化
- 对于标准的JDK线程池的处理逻辑
	- 当新任务进来，如果当前**工作线程数<核心线程数**，则创建新线程处理
	- 如果**工作线程数>=核心线程数**，任务就会被放入阻塞队列进行等待
	- 如果阻塞队列也满了，且工作线程数小于最大线程数时，才会创建新的线程
	- 如果工作线程数已达到最大线程数，且阻塞队列已满，则执行拒绝策略
- Tomcat线程池的改进策略
	- Tomcat认为Web请求时IO密集型任务，更希望优先创建线程来快速处理，而不是让请求在队列中长时间等待
	- 为此，Tomcat重写了任务队列的入队逻辑。当工作线程数已达到最大线程数时，它不会立即执行拒绝策略，而是会再次尝试将任务放入队列
	- 只有当这个再次入队的尝试也失败时，才会真正触发拒绝策略
当线程和队列都达到上限后，新请求将由配置的`RejectedExecutionHandler`处理。Tomcat提供了以下几种策略
- **AbortPolicy**：直接抛出`RejectedExecutionException`异常。适用于希望明确感知系统过载，进行监控和告警
- **CallerRunsPolicy**：由调用者线程同步执行该任务。该策略可以有效减缓请求接收速度，保护系统，避免任务丢失
- **DiscardPolicy**：静默丢弃当前无法处理的任务。适用于允许少量请求丢失的非关键业务
- **DiscardOldestPolicy**：丢弃队列中等待最久的一个任务，然后再次尝试提交当前任务。适用于希望优先处理最新请求的场景
## 62. GC Root对象都有哪些
**虚拟机栈引用的对象**：这是最常见的GC Root。指当前正在运行的方法中的局部变量和方法参数所引用的对象。例如
```java
public void method() {
	Object obj = new Object();
}
```
此时obj就是一个GC Root。方法执行完毕，栈帧弹出，obj失去Root身份，对象就能被回收
**静态属性引用的对象**：指被`static`修饰的变量所引用的对象。只要这个类没有被卸载（即类加载器存活），静态变量引用的对象始终都是GC Root
```java
private static UserService service = new UserServiceImpl();
```
这里的service就是GC Root对象
**常量**：指被`final`修饰的常量，以及字符串常量池中的字符串引用所指向的对象
```java
private static final String NAME = "constant";
```
这里的字符串对象就是GC Root
**本地方法栈(JNI)引用的对象**：指通过JNI在本地方法中创建或持有的全局对象引用和局部变量引用
调用`System.currentTimeMillis()`这类底层Native方法时，其内部持有的Java对象引用
**活跃的Thread线程**：所有处于运行状态的线程对象本身就是GC Root。线程池有的栈帧中的局部变量也属于GC Root的一部分。这就是为什么如果线程不结束，其内部创建的对象一直不会被回收
**同步监视器持有的对象**：被`synchronized`关键字修饰的锁对象，会被GC Root持有
...
## 65. Redisson分布式锁的原理
Redisson的分布式锁，可以理解为在Redis这个高性能的中间件上，通过**数据结构设计、Lua脚本原子性、看门狗自动续期以及发布/订阅通知**等一系列机制，构建了一个健壮、高可用的分布式锁服务
Redisson的锁信息在Redis中是以Hash数据结构存储的。通过`Key：锁的名称`、`Field：持有锁的客户端唯一标识`、`Value：该线程的重入次数`。这种设计既存储了锁的持有者，又能轻松实现锁的可重入性
Redisson枷锁并非简单的`SETNX`命令，而是通过执行一段Lua脚本来保证整个操作的原子性
- 首先检查所是否存在，使用`exists`命令检查Key是否存在，若不存在，则执行`hincrby`设置重入次数为1，并用`expire`设置过期时间
- 检查锁是否是当前线程持有，若锁存在，则使用`hexists`检查Field是否为当前线程ID。若是，则执行`hincrby`将重入次数加1，并重置过期时间
- 如果锁已经被其他线程持有，则直接返回锁的过期时间
**看门狗机制**
看门狗是Redisson解决锁过期事件难题的核心设计。当使用`lock()`方法且未指定锁的过期时间时，默认启动看门狗机制
- 加锁成功后，会启动一个后台定时任务
- 该任务默认每隔10秒（锁默认超时时间30秒的1/3）执行一次
- 每次执行时，会检查锁是否仍被当前线程持有。如果是，则通过Lua脚本将锁的过期时间重新设置为30秒
- 当线程主动调用了`unlock()`释放锁时，看门狗任务也会被取消。若持有锁的进程意外崩溃，看门狗任务随之终止，锁也会在30秒后自动释放，避免了死锁
**锁互斥与重试**
当线程尝试获取已经被占用的锁时，并不会无休止的自旋，而是采用高效的发布订阅模式
- 订阅通知：获取锁失败后，线程会订阅与该锁相关的特定频道
- 进入等待：线程进入阻塞等待状态
- 唤醒重试：当锁被释放时，持有锁的线程会向该频道发布一条解锁消息。所有订阅了此频道的等待线程收到消息后，会被唤醒并重新尝试获取锁
**解锁流程**
解锁流程同样通过Lua脚本保证原子性
- 检查锁的Hash中是否存在当前线程的Field。若不存在，说明锁已失效或已被他人持有，直接返回
- 若存在，则将重入次数-1。如果-1后值大于0，表示线程仍持有该锁，则重置锁的过期时间。如果-1后的值等于0，表示线程已经完全释放锁，则执行del命令删除锁key，并发布解锁消息，唤醒等待的线程
## 66. JVM由哪些部分组成
JVM本质上是一个执行Java字节码的抽象计算机。通常由四大核心子系统组成
**类加载器子系统**
负责将`.class`字节码文件加载到内存中，并进行后续处理。它不仅仅是读文件，而是包含了三个核心阶段
- **加载**：通过类的全限定类名获取二进制字节流，将其转化为方法区中的静态数据结构，并生成对应的`Class`对象
- **链接**：包含验证、准备、解析三个阶段。验证阶段是为了保证字节码安全；准备阶段是为静态变量分配内存并设置零值、解析阶段是将符号引用替换为直接引用
- **初始化**：执行类的构造器`<clinit>`方法，真正为静态变量赋初始值
这个系统有一个关键机制：**双亲委派模型**
**运行时数据区**
这是JVM最核心的部分，根据线程是否私有，分为两大类
第一部分是线程私有，生命周期和线程的生命周期一致
- **程序计数器**：记录当前线程正在执行的字节码指令地址。它是内存中唯一一个不会发生OOM的区域
- **虚拟机栈**：描述Java方法执行的内存模型。每个方法执行时会创建一个栈帧，存储局部变量表、操作数栈、动态链接和方法出口。此区域常见`StackOverflowError`和`OutOfMemoryError`
- **本地方法栈**：与虚拟机栈类似，但是是专为执行Native方法服务
第二部分是线程共享的，生命周期和JVM一致
- **堆**：JVM中最大的内存区域，所有对象实例和数组都在这里被分配内存。它是垃圾回收的主要战场，分为新生代和老年代。新生代又分为Eden区和Survivor区。Survivor区又分为S0和S1区
- **方法区**：存储已被加载的类信息、常量、静态变量，即时编译器编译后的代码等数据。在JDK1.8之后，HotSpot虚拟机使用元空间取代了永久代，元空间不再使用JVM堆内存，而是使用本地物理内存，因此默认上线受制于机器的物理内存，大大减少了`PermGen OOM`错误
**执行引擎**
负责将加载到内存中的字节码解释/编译为机器指令，交给操作系统执行
- **解释器**：逐行将字节码翻译成机器码并执行。启动快，但是执行效率低
- **即时编译器**：针对热点代码，将其直接变以为本地机器码并缓存，提升运行效率。现代JVM通常采用混合模式（解释+JIT编译）
- **垃圾回收器**：自动管理堆内存，回收不再使用的对象。常见的回收算法有：标记整理、复制、标记清除，具体的实现包括Serial、Parallel、CMS、G1以及ZGC
**本地方法接口和本地方法库**
**JNI(Java Native Interface)**：作为Java层和操作系统底层/硬件之间的桥梁。它允许Java代码调用其他语言编写的本地库，也允许其他语言调用Java代码。当执行引擎遇到`native`关键字修饰的方法时，会通过JNI去调用操作系统提供的底层本地方法来完成具体内容
## 67. HashMap扩容
HashMap的扩容是由负载因子控制的，默认是0.75。意思就是，当元素的数量超过容量\*0.75时触发扩容。扩容的规则也很简单，容量直接翻倍，从16变32,32变64，始终保持2的幂次。扩容后所有元素都需要重新分配位置，这个过程叫做rehashing
**rehashing**
在JDK1.7时，每个元素是重新利用hash计算索引位置一个一个搬迁过去的。但是在JDK1.8时对这个做了优化。因为数组的长度是2的n次方，那么扩容前后数组长度之间的差别就是扩容后高位多了一个1，而我们通过key的hash值定位其数组位置使用的方法是(n-1)&hash。例如`16-1=15，二进制为0000 1111`，`32-1=31，二进制为0001 1111`，重点就在于key的hash值从右往左数第五位是否是1，如果是1就需要搬迁到新的位置，新的位置的值是`原位置+旧数组容量`，如果hash值的高位是0，那么这个元素依然在数组原位置
## 68. 线上频繁Full GC该如何排查以及排查手段
**紧急止血，恢复服务**
当频繁触发Full GC导致响应迟缓或不可用时，首要任务就是快速恢复服务，避免故障影响范围扩大
在重启前，使用`jmap`命令生成堆转储文件（`jmap -dump`操作可能会导致STW，在服务完全卡死的情况下可能执行失败，需评估风险）。然后立即重启Java进程，使服务恢复正常。如果重启后问题依旧，可以考虑通过网关对该服务进行限流或降级减少瞬时压力
**分析现象，收集证据**
服务恢复后，需要系统性收集证据，定位根因
首先需要确保JVM已经开启GC日志，这是分析问题的最重要的数据源。使用`jstat`工具实时观察GC状况，确认Full GC是否真的频繁。需重点关注FGC（Full GC次数）是否快速增长，以及FGCT（Full GC总耗时）是否过长。然后结合`jstat`输出和老年代的使用率，可以初步判断问题类型
- **Full GC后，老年代使用率几乎不降**：可能是内存泄露导致，可以生成Heap Dump，用MAT等工具进行分析
- **老年代使用率突然飙升**：可能是大对象直接进入老年代。可以检查代码中是否有一次性加载大量数据
- **Full GC频繁，但堆内存使用率不高**：可能是元空间不足。可以检查`jstat -gc`中的MU（已使用）和MC（容量）
- **GC日志中看到`System.gc()`调用**：代码中显式调用了`System.gc()`。全局搜索代码，或添加`-XX:+DisableExplicitGC`参数临时禁用
- **YGC次数少，但FGC频繁**：新生代过小，检查`-Xmn`等新生代相关参数设置
**深入分析，定位根因**
- 分析内存泄漏：这是最常见的原因。可以使用MAT或JProfiler等工具打开Heap Dump文件。查看`Leak Suspects`报告，MAT会自动分析并给出可以的内存泄漏点。查看`Histogram`，按对象实例数或内存占用大小排序，找出占用最多的对象类型。查看`GC Roots`，对可疑对象右键选择`Path to GC Roots`，排除弱引用/软引用，查看是什么阻止了它被回收
- 排查大对象问题：使用`jmap -histo:live <pid>`命令，可以查看当前存活对象统计，快速定位占用内存过大的类型。检查代码中是否有一次性从数据库/文件中读取大量数据、创建超大数组/字符串等情况
- 排查元空间/显式GC问题：使用`jstat -gc <pid>`监控元空间使用情况。使用`jmap -clstats <pid>`查看类加载器统计信息，定位类加载泄露。全局搜索代码中的`System.gc()`调用，并评估是否必要
**制定方案，彻底根治**
- 内存泄露：修复代码，例如及时清理缓存、关闭资源、在finally块中清理ThreadLocal等
- 大对象问题：优化代码，分页查询、使用流式处理、压缩数据或将处理逻辑移出JVM内存
- 参数配置不当：根据业务特点调整JVM参数，如`-Xmx`、`-Xmn`、`-XX:MaxMetaspaceSize`等。对于G1回收期，可以关注`-XX:MaxGCPauseMillis`和`-XX:InitiatingHeapOccupancyPercent`等参数
- 显式GC：移除不必要的`System.gc()`调用，或通过`-XX:DisableExplicitGC`参数忽略这个代码
## 69. Full GC触发条件
Full GC是Java虚拟机中代价最高的垃圾回收操作，它会回收整个堆内存，通常伴随着较长时间的STW停顿，其触发条件较复杂，主要分为以下几大类
**内存空间不足**
- **老年代空间不足**：当对象从新生代晋升到老年代，或创建的大对象直接分配在老年代时，如果老年代剩余空间不足，就会触发Full GC
- **方法区空间不足**：JDK8之前，永久代用于存放类信息、常量池等，空间不足会触发Full GC。JDK8开始，永久代被元空间取代，当元空间达到其设定阈值时，也会触发Full GC
**新生代垃圾回收过程中的特殊情况**
- 在执行Young GC前，JVM会检查老年代剩余的连续空间是否足够。如果老年代空间<历次Young GC后晋升到老年代的对象的平均大小，JVM会认为本次Young GC后老年代可能放不下晋升的对象，从而直接触发Full GC
- 在执行Young GC之后，有大量的对象需要晋升到老年代，但老年代空间不足，导致晋升失败，此时会触发Full GC
- 在Young GC过程中，如果Survivor区无法容纳存活的对象，需要依赖老年代进行担保，而担保失败（即老年代空间也不足）时，会触发Full GC
**特定垃圾回收器的并发失败**
对于像CMS这样的并发垃圾回收器，在并发清理阶段，用户线程仍在运行，可能会产生新的垃圾
如果CMS在并发回收期间，老年代空间不足以容纳新产生的对象，就会发生并发模式失败，JVM将暂停所有用户线程，并启用Serial Old垃圾回收器进行一次全局的、STW的Full GC
**人为触发**
- 在代码中显式调用`System.gc()`或`Runtime.getRuntime.gc()`方法，会向JVM建议进行一次Full GC。但JVM不保证一定会执行
- 使用`jmap -dump`等诊断工具时，也可能会触发Full GC
## 70. 公有桶和私有桶的区别，链接有什么区别
**私有桶**
私有桶的读写都仅限于授权用户。只有存储桶的创建者以及被授权的账号可以读取对象。不支持匿名访问，任何未经身份验证的访问都会被拒绝。**访问链接是带签名的临时链接**。适用于存放私密数据，如用户资料，公司内部文件、备份等
**公有桶**
公有桶的读是所有人都可以读取的，写入权限是只有存储桶的创建者以及被授权的账号可以写入对象。支持匿名访问，匿名用户可以直接通过链接访问和下载文件。**访问链接是不带签名的永久链接**。适用于存放需要公开分享的数据，如网站图片、静态资源等
私有桶的访问URL必须包含签名信息，这个签名是请求者的身份凭证，证明其有权限访问该文件。这类链接通常具有时效性，默认有效期为2小时，过期后需要重新生成
公有桶的访问链接，由于允许匿名访问，其访问URL不带任何签名参数的对象地址。这个链接本身是永久有效的，任何知道链接的人都可以随时访问
## 71. G1 垃圾回收器为何需大内存配置才能让性能更好
G1垃圾回收器专为大内存、低延迟的场景设计。它在小内存下表现不佳，主要是其精妙的**分区机制**和**停顿预测模型**在大内存下才能充分发挥优势
G1的大内存通常指堆内存大小在6GB以上。它就是为了解决传统垃圾回收器在处理大内存时的两大痛点
- **Parallel GC的雪崩式停顿**：追求高吞吐量，但一次Full GC可能导致长达数秒甚至数分钟的STW停顿
- **CMS的并发失败与内存碎片**：虽然并发执行以减少停顿，但可能因浮动垃圾过多而退化成长时间的Full GC。且其标记-清除算法会产生大量的内存碎片，即使总内存足够，也可能因为没有连续的大块空间而触发Full GC
G1解决这些问题的核心，就是基于Region的内存布局
G1不再将堆内存划分为固定的年轻代、老年代等连续大块，而是将其分割成大量大小相等、不连续的独立区域
- 角色动态化：每个Region可以动态扮演Eden、Suvivor、Old等角色，带来了极大的灵活性
- 回收粒度化：GC的单位不再是整个代，而是若干个Region的集合。G1会优先回收垃圾最多的Region，因此得名`Garbage-First`（垃圾优先）
**为什么G1在大内存下性能更好**
- **停顿时间可预测**：G1内部有一个停顿预测模型，会根据历史回收数据，精准预测在用户设定的暂停时间目标内，能回收多少个Region。它只回收一部分Region，从而将每次GC的停顿时间控制在目标范围内。**在大内存下，G1有足够多的Region可供选择，可以更从容地挑选，在有限时间内回收最多的内存，实现收益最大化。** 而在小内存下，Region数量少，选择空间少，G1复杂的调度算法反而可能成为负担
- **有效消除内存碎片**：CMS的标记清除算法会产生碎片。而G1采用的是复制算法，在回收Region时，会将存活对象复制到另一个Region中，从而实现内存压缩，从根本上消除了碎片问题。在大内存、高并发的长期运行应用中，这种优势极为
## 72. 一个服务实例注册到Nacos中，需要向Nacos中注册哪些信息
**连接Nacos服务端的信息**
...
## 73. 编译执行与解释执行的区别
- **编译执行**：编译器将整个源代码程序提前翻译成目标平台能直接运行的机器码。优点是执行效率高，因为运行时无需再翻译；缺点是**跨平台性差**，针对不同系统需要不同的可执行文件
- **解释执行**：解释器读取一句源代码，就翻译并执行一句，不生成独立的可执行文件。优点是**跨平台性好**，只要有对应的解释器就能运行；缺点是执行效率低，因为每次运行都需要重复翻译
JVM默认采用混合模式，核心是**先解释，后编译**
- 程序启动时，JVM的解释器会立即开始工作，逐条将字节码翻译为机器码并执行。这保证了程序能快速启动，无需等待漫长的编译过程
- 程序运行过程中，JVM会持续监控代码的执行情况。当发现某个方法或代码块被频繁调用时，JIT编译器就会接入
- JIT编译器会讲这些热点代码以**方法为单位**，编译成经过高度优化的本地机器码，并缓存起来供后续直接调用。这样，程序的整体执行效率会越来越高
## 74. JVM有哪些OOM的情况
- **Java堆内存溢出**：这是最常见的情况。要么是内存泄露（对象一直被GC Roots引用，无法回收，如忘记关闭数据库连接、集合中对象只增不减）；要么是内存膨胀（单个请求创建了超大对象或容器，如一次性加载上亿条数据到List中）
- **方法区/元空间移出**：方法区存储类的元数据、静态变量、常量池等。当程序动态生成大量类时容易触发。例如频繁使用CGLib动态代理，JSP热部署、或Spring/Mybatis加载了大量增强类
- **本地线程栈溢出**：这是操作系统的内存不足以给JVM创建新的本地内存。当程序创建了海量线程、且每个线程栈占用内存累积超过系统剩余内存时触发。
- **直接内存溢出**：使用`ByteBuffer.allocateDirect()`分配堆外内存时，未受GC直接管理，受`-XX:MaxDirectMemorySize`限制。如果NIO框架使用直接内存不当，或未及时释放，就会溢出
- **GC效率低下导致溢出**：JVM的一个保护机制。当JVM话费了超过98%的时间执行GC，却只能回收不到2%的堆外内存是，JVM会主动抛出此异常，防止程序陷入无限死循环式的GC卡顿
- **超大数组请求溢出**：程序试图分配一个大小超过JVM允许上限的数组
## 75. 什么是直接内存
直接内存，也叫堆外内存。这是JVM进程内部，但不受JVM垃圾回收管理的一块原生内存
它存在的意义。传统的Java IO读写数据需要经历两次内存拷贝，而直接内存允许Java程序直接访问操作系统内核空间的一块内存。NIO的`ByteBuffer.allocateDirect()`分配的就是这块内存。当进行网络或文件读写时，数据直接在OS内核空间和直接内存之间传输，省去了JVM堆内存这个“中间商”
这就是**零拷贝**技术，在高并发网络框架中性能极高，大大减少了CPU拷贝上下文切换的开销
直接内存不参与Young GC或Old GC，它的回收依赖于`sun.misc.Cleaner`（虚引用机制）
- 当`DirectByteBuffer`对象被GC回收时，关联的`Cleaner`对象会触发一个`run()`方法，通过`Unsafe.freeMemory()`释放掉堆外的真实内存
- 如果堆内存一直很充足，导致`DirectByteBuffer`对象迟迟不被GC回收，直接内存可能会被撑爆
**直接内存适用于数据量较大且生命周期较长、网络IO密集型的应用，不适用于频繁创建和释放的小对象，或业务逻辑简单的普通Web后台**
## 76. 什么是常量池
常量池主要分为三个层次：**Class文件常量池、运行时常量池和字符串常量池**
- **Class文件常量池**：这是一场编译期间生成的静态表格。主要储存两种东西：字面量（代码中的文本字符串，如`hello`、final常量值，如`final int a = 10`）。符号引用（类和接口的全限定类名、方法名、字段名等）。这个池子是给JVM加载类时准备的原料，本身不占用JVM运行时内存，而是直接存储在硬盘中
- **运行时常量池**：当JVM把`.class`文件加载进内存后，Class文件常量池里的内容就会被搬到方法区，变成运行时常量池。它不在局限于编译器的内容。运行时也可以往里面放新的常量，例如`String.intern()`方法
- **字符串常量池**：这是专门用于存字符串对象的一个池子。它是一个全局的HashMap，全局只有一份，所有线程共享。它的Key时字符串的哈希值，Value是指向堆中具体字符串对象的引用。当写`String s = "abc"`时，JVM会先去字符串常量池里查找有没有内容为`"abc"`的引用。如果没有，则在堆中创建一个`"abc"`对象，并把引用放进常量池中；如果有，则直接返回该引用，不会重复创建新对象
## 77. 类加载器
JVM启动时，会预先创建三个层级分明的类加载器
- **启动类加载器**：负责加载`<JAVA_HOME>/lib/rt.jar`等核心包。主要负责加载Java核心类库。无父加载器
- **扩展类加载器**：负责加载`<JAVA_HOME>/lib/ext/*`或系统变量指定路径，主要负责加载`javax.*`等扩展库。父加载器为Bootstrap
- **应用类加载器**：负责加载`CLASSPATH`环境变量指定的路径。主要负责加载用户自己的业务类。父加载器为Extension
**双亲委派机制**
当一个类加载器收到加载请求时，它不会立刻自己加载，而是先将请求委派给父类加载器去处理，直到顶层的Bootstrap加载器尝试加载。如果父类加载器无法加载，则子类加载器才会自己加载
双亲委派机制的核心优势在于
...
## 79. Java 线程池核心线程数在运行过程中能修改吗？如何修改？
可以修改，主要通过`ThreadPoolExecutor`提供的`setCorePoolSize(int corePoolSize)`方法实现。以下是该方法在运行时调整线程数量的具体行为
**增加核心线程数**
- **创建新线程**：线程池会尝试创建新的核心线程，直到线程数达到新的设定值
- **处理积压任务**：如果任务队列中有等待处理的任务，新创建的线程会立即从队列中取出任务并执行
**新线程的创建是按需的，通常在有新任务提交时才会触发，并非调用方法后立即全部创建**
**减少核心线程数**
- **不强制终止**：多余的核心线程不会被立即强制销毁
- **空闲回收**：这些多余的线程会被标记为空闲状态。如果它们的空闲时间超过了`keepAliveTime`设定值，通常会被回收
- **核心线程超时**：通常情况下，核心线程即使空闲也不会被回收。但如果通过`allowCoreThreadTimeOut(true)`开启了核心线程超时，那么所有空闲的核心线程在超时后都可能会被回收
**注意事项**
- 修改后的核心线程数不应大于最大线程数，否则可能导致逻辑不合理。最大线程数也可通过`setMaximumPoolSize(int)`方法动态调整
- 如果任务队列容量过大，增加核心线程数的效果可能不明显，因为新任务可能优先进入队列等待，而非创建新线程
- 在生产环境中，常将线程池参数托管到配置中心，实现运行时动态调整而无需重启应用
## 80. AQS
AQS全称`AbstractQueuedSynchronizer`，是JUC包里锁和同步器的基础框架。`ReentrantLock`、`Semaphore`、`CountDownLatch`、`ReentrantReadWriteLock`这些类都是基于它实现的
AQS的核心就两样东西，一个volatile int类型的state变量，一个双向链表实现的FIFO同步队列
state代表资源状态，具体含义由子类定义。ReentrantLock里state表示锁被重入了多少次，Semaphore里state表示剩余多少许可，CountDownLatch里表示还剩多少计数
线程想获取资源，先CAS修改state，成功了就拿到资源，失败了就包装成Node节点加入队列尾部，然后park挂起等待。前面的线程释放资源后会unpark唤醒队列的后继节点
AQS有两种模式
- **独占模式**：同一时刻只有一个线程能持有资源。ReentrantLock就是独占模式，state为0表示没被占用，大于0表示被某个线程占用
- **共享模式**：多个线程可以同时持有资源。Semaphore允许最多N个线程同时访问，state初始值设为N，每个线程获取时state减1，释放时加1，减到0就没有许可了
**AQS为什么用CLH队列的变体而不是普通队列**
原版CLH是自旋等待前驱节点释放锁，每个线程只需要关注自己前驱的状态，避免了所有线程CAS同一个变量导致总线风暴。AQS把自旋改成了park阻塞，这样长时间等锁不会白白烧CPU。但阻塞了就没办法自己检测前驱状态，所以改为双向链表，让前驱主动unpark唤醒后继
## 81. Cookie、Session、Token有什么区别
- Cookie是客户端的本地存储机制。它只是一小段文本数据，由服务器通过`Set-Cookie`响应头下发，浏览器会自动保存，并在后续请求中通过`Cookie`请求头自动携带
- Session是服务端的有状态认证机制。服务器在内存中维护一个全局Map（键为SessionId，值为用户数据）。SessionId通常就是通过Cookie传递给客户端的。
- Token是客户端持有的无状态凭证。它是一串经过加密签名的字符串，包含了用户身份信息。它不强制依赖Cookie，通常放在HTTP请求头`Authorization`中手动传递
**CSRF风险**
因为浏览器会自动携带Cookie，恶意网站可以通过表单或图片请求自动携带你的Cookie发送请求，防御需要加上CSRF Token
**XSS风险**
因为Token需要前端手动读取并注入Header，如果页面存在XSS漏洞，攻击者可以直接窃取Token。防御需要多用户输入做严格过滤。Token也可以放在Cookie中并设置`HttpOnly`和`SameSite=Strict`，可以同时防御XSS和CSRF，但这又回到了依赖Cookie的模式
## 82. 原子性、可见性、有序性
原子性、可见性、有序性是Java并发编程中的三大核心特性，任何并发bug基本都能归到这三个类里面
**原子性**指一个操作要么全部执行完，要么全部不执行，中间不会被其他线程打断。比如说`i++`这个操作，实际上是读取、加1、写回三个步骤，多线程环境下就可能出现问题
**可见性**指一个线程修改了共享变量，其他线程能立刻看到最新值。CPU有自己的高速缓存，线程修改的值可能还在缓存中没刷回主内存，别的线程就读到了旧值
**有序性**指程序执行顺序和代码写的顺序一致。编译器和CPU为了性能会对执行做重排序，单线程下没问题，多线程可能就会出现一些bug
**如何保障原子性**
Java里保证原子性主要靠两种方式：锁和CAS
`synchronized`和`Lock`是最直接的手段，进入临界区的线程独占资源，其他线程只能干等着。但锁的开销都不小，线程切换、阻塞唤醒都是重量级操作
CAS是一种乐观锁思路，底层依赖CPU的`cmpxchg`指令。比如AtomicInteger的`incrementAndGet`，它会不断尝试比较当前值是否等于预期值，等于就更新，失败就重试。CAS避免了线程阻塞，但在竞争激烈时会疯狂自旋，CPU空转
**可见性的底层原理**
可见性问题的根源就在于CPU缓存。现代CPU都有L1、L2、L3多级缓存，每个核心都有自己的L1/L2，线程读写变量时优先操作缓存。如果线程A在CPU0上修改了变量，值还在L1缓存里，线程B在CPU1中读到的还是旧值
volatile的作用就是强制刷新缓存。写volatile变量时，JVM会插入StoreStore屏障和StoreLoad屏障，把缓存里的数据刷回主内存；读volatile变量时，会插入LoadLoad屏障和LoadStore屏障，强制从主内存读取
synchronized也能保证可见性。线程退出synchronized块时，会把所有修改刷回主内存；进入synchronized块时，会清空本地缓存，强制从主内存重新加载。所以synchronized块里的代码不需要额外添加volatile
final字段也有可见性保障。JVM保证对象构造完后，final字段的值对其他线程可见，不需要额外同步。这就是为什么String的value数组是final的
**有序性和指令重排**
编译器和CPU都会做指令重排序，目的是充分利用CPU流水线，提高执行效率
编译器重排是在生成字节码或机器码时调整指令顺序。比如两条不相关的赋值语句，编译器可能通过调换顺序来优化寄存器使用
